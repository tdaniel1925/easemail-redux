/**
 * Reply to Email API Route
 * Handles replying to emails via provider APIs
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { messageId, body_html, to_all = false, cc, bcc } = body;
    ...
    await supabase.from('usage_tracking').insert({
      user_id: user.id,
      feature: to_all ? 'email_reply_all' : 'email_reply',
      count: 1,
